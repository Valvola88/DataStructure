...
typedef struct aiv_list_node dlist_node;
typedef struct aiv_int_node int_dlist_node;
...
{
    const dlist_node* to_remove = list_get_pos(head, pos);
...
int main(int argc, char** argv)
{
    ...
    aiv_print_list(head, 'i');
    printf("(%i)\n",list_get_len(head));
    printf("{%i}",((int_dlist_node*)list_get_pos(&head, 3))->value);
}